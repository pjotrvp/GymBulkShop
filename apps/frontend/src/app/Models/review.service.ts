...
export class ReviewService extends EntityService<Review> {
  constructor(protected override http: HttpClient) {
    super(environment.apiUrl, http, 'reviews');
  }
  reviews: Review[] = [
    {
      _id: '1',
  ...
  getReviewIdBySupplementId(supplementId: string) {
    return this.reviews.find((review) => review.supplementId === supplementId)!
      ._id;
  }
  ...
  updateReview(review: Review) {
    this.reviews = this.reviews.map((r) => (r._id === review._id ? review : r));
  }
}
